	...
	class Xhtml2Pdf
     ...
     class_path = ""
          ...
          class_path << "#{options[:classpath_separator]}#{jar}"
			...
			raise("Unexpected error in write_pdf while executing Java command: #{command}") unless result
			result
		end
		...
		def self.encrypt_pdf(options,output_file_name,password)
			...
			result = system(command)
			raise("Unexpected error in encrypt_pdf while executing Java command: #{command}") unless result
			result
		end
